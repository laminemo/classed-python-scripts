...
for result in twitter_accounts.find():
    ...
    for i in range(len(division_threshold) - 1):
        ...
        if result["followers_count"] < division[i+1]:
            ...
            twitter_accounts.update_one({"_id": result['_id']}, {"$set": {"division": i + 1}})
            twitter_stats.update({"author_id" : result['twid']}, {"$set" : {"division" : i + 1}})

    if result["followers_count"] >= division_threshold[len(division_threshold) - 1]:

        twitter_accounts.update_one({"_id": result['_id']}, {"$set": {"division": len(division_threshold)}})
        twitter_stats.update_many({"author_id" : result['twid']}, {"$set" : {"division" : len(division_threshold)}})

    #### ajout de la métrique follow_rate :

    twitter_accounts.update_one({"_id" : result['_id']}, {"$set" : {"follow_rate" : result["followers_count"] / result["following_count"]}})
...
for result in collection.find():
    ...
    try:
        ...
        collection.update_one({"_id": result['_id']}, {"$set": {"engagement_rate": tx}})

    except:

        collection.update_one({"_id": result['_id']}, {"$set": {"engagement_rate": 0}})
...
for user in twitter_accounts.find():
    ...
    try:
        twitter_accounts.update_one({"_id": user['_id']}, {"$set": {"follower_list": user["follower_list"] + str(user["followers_count"]) + "_" + user["last_stats"]}})

    except:
        twitter_accounts.update_one({"_id": user['_id']}, {"$set": {"follower_list": str(user["followers_count"]) + "_" + user["last_stats"]}})
        ...
        if len(list(twitter_accounts.aggregate(pipeline))) != 0:
            ...
            for user in twitter_accounts.find({"division" : div}):
                ...
                if (max - min) != 0:
                    ...
                    twitter_accounts.update_one({"_id": user['_id']}, {"$set": {feature + "_normal": 0}})
...
tweet_feature_to_normalize = ["organic_metrics_user_profile_clicks", "engagement_rate", "organic_metrics_impression_count"]
        ...
        if len(list(twitter_stats.aggregate(pipeline))) != 0:
            ...
            for tweet in twitter_stats.find({"division" : div}):
                if(max - min) != 0:
                    ...
                    twitter_stats.update_one({"_id": tweet['_id']}, {"$set": {feature + "_normal": 0}})
...
weights_list = {"follow_rate" : 0.1, "engagement_rate" : 0.2, "impressions" : 0.1, "tweet_count" : 0.1}
...
for user in user_collection.find():
    ...
    for media in media_collection.find():
        ...
        if media["author_id"] == user["twid"]:
            ...
            av_click = av_click + media["organic_metrics_user_profile_clicks_normal"]
            av_eng = av_eng + media["engagement_rate_normal"]
            av_impression = av_impression + media["organic_metrics_impression_count_normal"]
    ...
    if i != 0:
        ...
        twitter_accounts.update_one({"_id": user['_id']}, {"$set": {"av_eng" + "_normal": av_eng}})
        twitter_accounts.update_one({"_id": user['_id']}, {"$set": {"av_imp" + "_normal": av_impression}})
        twitter_accounts.update_one({"_id": user['_id']}, {"$set": {"av_click" + "_normal": av_click}})

    else:

        twitter_accounts.update_one({"_id": user['_id']}, {"$set": {"av_eng" + "_normal": 0}})
        twitter_accounts.update_one({"_id": user['_id']}, {"$set": {"av_imp" + "_normal": 0}})
        twitter_accounts.update_one({"_id": user['_id']}, {"$set": {"av_click" + "_normal": 0}})




    score = 0.1 * tweet_count + 10 * av_eng + 0.1 * av_impression + 10 * follow_rate + 0.1 * av_click

    if verified :
        score = score + 5

    user_collection.update_one({"_id": user['_id']}, {"$set": {"score": int(score)}})

    influence_accounts.update_one({"twitter" : user["twid"]}, {"$set" : {"twitter_score" : int(score)}})
...
feature_list = ["impressions_normal", "engagement_rate_normal", "organic_metrics_user_profile_clicks_normal", "follow_rate_normal", "tweet_count_normal"]
...
for user in twitter_accounts.find():
    ...
    feature_list.append(0.2 * user["organic_metrics_user_profile_click_normal"])
    ...
    user_collection.update_one({"_id": user['_id']}, {"$set": {"topsis_score": int(score)}})

    influence_accounts.update_one({"twitter": user["twid"]}, {"$set": {"twitter_topsis_score": int(score)}})

    influence_accounts.update_one({"twitter": user["twid"]}, {"$set": {"twitter_division": user["division"]}})

    influence_accounts.update_one({"twitter": user["twid"]}, {"$set": {"twitter_followers": user["followers_count"]}})
